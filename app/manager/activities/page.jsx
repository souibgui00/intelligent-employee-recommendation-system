...
export default function ManagerActivitiesPage() {
  ...
  const ActivityCard = ({ activity, deptEmployees, enrollments, getTypeIcon, formatActivityDay, approveActivity, setRejectingActivity, setRejectionModalOpen, navigate }) => {
    ...
    return (
      <article className="bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md hover:border-primary/30 transition-all group overflow-hidden flex flex-col h-full" aria-label={activity.title}>
  ...
  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <section className="flex flex-col bg-[#F8FAFC] min-h-screen" aria-label="Manager activities list">
