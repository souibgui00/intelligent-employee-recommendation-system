...
export default function AdminAuditLogsPage() {
    ...
    return (
        <main id="main-content" className="flex flex-col bg-[#F8FAFC] min-h-screen" aria-label="Admin audit logs page">
            <DashboardHeader title="Audit & Traceability" description="System mutation logs enforcing US24 traceability compliance" />

            <section className="flex-1 p-8 space-y-6" aria-label="Audit logs content">
                <section className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden" aria-labelledby="mutation-log-heading">
                    <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div className="flex items-center gap-3">
                            <Shield className="w-5 h-5 text-primary" aria-hidden="true" />
                            <div>
                                <h2 id="mutation-log-heading" className="text-sm font-black text-slate-900 uppercase tracking-widest">Mutation Log</h2>
                        ...
                        >
                            {loading ? <Loader2 className="w-3 h-3 animate-spin" aria-hidden="true" /> : <Activity className="w-3 h-3 text-primary" aria-hidden="true" />}
                            Refresh
                        </button>
                    </div>
                    ...
                    <div className="p-4 bg-white border-b border-slate-100 flex flex-wrap items-center gap-4">
                        <div className="relative flex-1 min-w-[200px]">
                            <label htmlFor="audit-search" className="sr-only">Search by actor, action, or entity</label>
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" aria-hidden="true" />
                            <input
                                id="audit-search"
                                type="text"
                                placeholder="Search by actor, action, or entity..."
                                aria-label="Search by actor, action, or entity"
                                value={searchTerm}
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                            <Calendar className="w-4 h-4 text-slate-400 ml-2" aria-hidden="true" />
                            <select
                                value={dateFilter}
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                            <Database className="w-4 h-4 text-slate-400 ml-2" aria-hidden="true" />
                            <select
                                value={entityFilter}
                                        ...
                                        <tr key={log._id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <button
                                                    ...
                                                    aria-label={`Inspect audit record ${log._id}`}
                                                >
