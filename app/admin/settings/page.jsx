...
import { Settings, Users, Save, Building2, Bell, Sparkles } from "lucide-react"
...
export default function SettingsPage() {
  ...
  return (
    <main id="main-content" className="flex flex-col bg-[#F8FAFC] min-h-screen" aria-label="System settings page">
      <DashboardHeader title="System Settings" description="Configure organizational parameters and user permissions." />

      <section className="flex-1 p-8 max-w-7xl mx-auto w-full space-y-8 animate-in fade-in duration-500" aria-label="Settings content">
        <section className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-12" aria-labelledby="settings-page-title">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <Button
              onClick={handleSave}
              aria-label="Apply all settings changes"
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-white border text-slate-500 border-slate-200 p-1 rounded-2xl h-14 w-fit shadow-sm" aria-label="Settings categories">
            <TabsTrigger value="general" className="px-8 font-bold text-xs rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white transition-all uppercase tracking-widest">
              <Settings className="h-4 w-4 mr-2" aria-hidden="true" /> General
            </TabsTrigger>
              ...
              <Card className="border-slate-200 shadow-sm rounded-2xl">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="company-name" className="text-xs font-bold uppercase tracking-widest text-slate-500">Company Name</Label>
                    <Input
                      id="company-name"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="industry-sector" className="text-xs font-bold uppercase tracking-widest text-slate-500">Industry Sector</Label>
                    <Input
                      id="industry-sector"
                      value="Assurance"
                      readOnly
                  ...
                  <CardContent className="space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <Label htmlFor="auto-recommendations" className="text-sm font-bold">Auto-Recommendations</Label>
                        <p id="auto-recommendations-description" className="text-xs text-slate-500">Suggest programs based on skill gaps automatically.</p>
                      </div>
                      <Switch
                        ...
                        aria-describedby="auto-recommendations-description"
                      />
                    </div>
                    <div className="space-y-2 pt-2">
                      <Label htmlFor="evaluation-frequency" className="text-xs font-bold uppercase tracking-widest text-slate-500">Evaluation Frequency</Label>
                      <Select
                        value={localSettings.evaluationFrequency}
                        onValueChange={(val) => setLocalSettings({ ...localSettings, evaluationFrequency: val })}
                      >
                        <SelectTrigger id="evaluation-frequency" className="rounded-xl bg-slate-50 border-slate-200" aria-label="Evaluation frequency">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
          ...
          <TabsContent value="notifications" className="space-y-6 outline-none">
            <Card className="border-slate-200 shadow-sm rounded-2xl overflow-hidden max-w-2xl">
              ...
              <CardContent className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="email-notifications" className="text-sm font-bold">Email Notifications</Label>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="in-app-alerts" className="text-sm font-bold">In-App Alerts</Label>
        ...
        {/* Global Stats Footer */}
        <footer className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-10 border-t border-slate-200" aria-label="System summary">
