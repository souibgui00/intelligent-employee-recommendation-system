...
export default function DepartmentsPage() {
    ...
    return (
        <main id="main-content" className="flex flex-col bg-[#F8FAFC] min-h-screen text-slate-600" aria-label="Admin departments page">
            <DashboardHeader title="Departments" description="Organize and manage organizational divisions and personnel structure." />

            <section className="flex-1 p-6 md:p-10 max-w-[1400px] mx-auto w-full animate-in fade-in duration-700 space-y-12" aria-label="Departments management content">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pt-4">
                    <div className="flex flex-wrap items-center gap-6 w-full lg:w-auto">
                        <div className="relative group w-full md:w-[500px]">
                            <label htmlFor="departments-search" className="sr-only">Search departments by name or code</label>
                            <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-primary transition-colors" aria-hidden="true" />
                            <input
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button
                                    variant="outline"
                                    aria-label="Open sort options for departments"
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <ArrowUpDown className="w-5 h-5 text-primary transition-colors" aria-hidden="true" />
                                    ...
                                    <Filter className="w-3 h-3 text-slate-300" aria-hidden="true" />
                                </Button>
                            </DropdownMenuTrigger>
                ...
                <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
                    {filteredDepts.map((dept) => {
                        ...
                        return (
                            <article key={dept.id || dept._id} className="group bg-white border-2 border-slate-50 rounded-[4rem] p-12 shadow-premium hover:shadow-2xl hover:-translate-y-4 transition-all duration-700 relative overflow-hidden flex flex-col justify-between" aria-label={`Department: ${dept.name}`}>
