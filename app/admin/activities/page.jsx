...
export default function AdminActivitiesPage() {
  ...
  return (
    <main id="main-content" className="flex flex-col bg-[#F8FAFC] min-h-screen text-slate-600" aria-label="Admin activities page">
      <DashboardHeader title="Activities" description="Manage your training programs, workshops, and sessions." />

      <section className="flex-1 p-6 md:p-10 max-w-[1400px] mx-auto w-full animate-in fade-in duration-700 space-y-12" aria-label="Activities management content">


        {/* Global Toolbar */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pt-4">
          <div className="flex-1 max-w-2xl relative group">
            <label htmlFor="activity-search" className="sr-only">Search activities by title or description</label>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  aria-label={activeFiltersCount > 0 ? `${activeFiltersCount} active activity filters` : "Open activity filters"}
                  className={cn(
                ...
                <div className="space-y-10">
                  <div className="flex items-center justify-between border-b border-slate-50 pb-6">
                    ...
                    {activeFiltersCount > 0 && (
                      <button type="button" onClick={() => { setFilterStatus("all"); setFilterType("all"); setFilterOccupancy("all") }} className="text-[10px] font-black text-rose-500 uppercase tracking-widest hover:underline" aria-label="Clear all activity filters">Clear All</button>
                    )}
                  </div>
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="h-16 w-[240px] px-8 rounded-[2rem] bg-white border-2 border-slate-50 shadow-premium hover:bg-slate-50 transition-all font-black text-[11px] uppercase tracking-widest text-slate-900 focus:ring-primary/20" aria-label="Sort activities">
          ...
          <div className="pt-24 space-y-16 border-t-2 border-dashed border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
              {activityRequests.map((request) => (
                <article key={request._id} className="group bg-white border-4 border-slate-50 rounded-[4.5rem] p-12 shadow-mega hover:border-primary/10 transition-all duration-700 relative overflow-hidden flex flex-col justify-between" aria-label={`Request: ${request.title}`}>
                  ...
                  <div className="flex gap-4 pt-12">
                    <Button type="button" className="flex-1 h-18 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[12px] tracking-[0.2em] uppercase rounded-[2rem] shadow-2xl shadow-emerald-600/30 active:scale-95 transition-all" onClick={() => handleApproveRequest(request._id)} aria-label={`Approve request for ${request.title}`}>
                      Approve
                    </Button>
                    <Button type="button" variant="outline" className="flex-1 h-18 border-2 border-rose-100 text-rose-500 font-black text-[12px] tracking-[0.2em] uppercase rounded-[2rem] hover:bg-rose-50 active:scale-95 transition-all" onClick={() => handleRejectRequest(request)} aria-label={`Reject request for ${request.title}`}>
                      Reject
                    </Button>
                  </div>
                </article>
        ...
        <Dialog open={reviewModalOpen} onOpenChange={setReviewModalOpen}>
          <DialogContent className="sm:max-w-2xl p-0 bg-transparent border-none shadow-none overflow-hidden" aria-labelledby="reject-dialog-title" aria-describedby="reject-dialog-description">
            <div className="bg-white rounded-[4rem] shadow-mega p-16 space-y-12 animate-in zoom-in-95 duration-500 relative ring-1 ring-slate-100">
              <div className="space-y-4">
                ...
                <DialogTitle id="reject-dialog-title" className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none">Reject Activity</DialogTitle>
                <DialogDescription id="reject-dialog-description" className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] opacity-60">Explain why you are rejecting this activity.</DialogDescription>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 pt-6">
                <Button type="button" variant="outline" className="flex-1 h-16 rounded-2xl font-black text-[11px] uppercase tracking-widest border-2 border-slate-100 hover:bg-slate-50 transition-all" onClick={() => setReviewModalOpen(false)} aria-label="Cancel rejection">Cancel</Button>
                <Button type="button" className="flex-1 h-16 rounded-2xl font-black text-[11px] uppercase tracking-widest bg-rose-600 hover:bg-rose-700 text-white shadow-xl shadow-rose-500/20 active:scale-95 transition-all" onClick={() => submitReview("REJECTED")} aria-label="Confirm rejection of activity request">Confirm Rejection</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
