...
export class AuditService {
  ...
  async logAction(data: {
    ...
    try {
      ...
      this.logger.error(`Failed to create audit log for ${data.action} on ${data.entityType} ${data.entityId}: ${err.message}`, err.stack);
  ...
  async findByEntity(entityType: string, entityId: string): Promise<AuditLog[]> {
    return this.auditLogModel.find({ entityType, entityId }).sort({ createdAt: -1 }).exec();
  }

  async findByActor(actorId: string): Promise<AuditLog[]> {
    return this.auditLogModel.find({ actorId } as any).sort({ createdAt: -1 }).exec();
  }
