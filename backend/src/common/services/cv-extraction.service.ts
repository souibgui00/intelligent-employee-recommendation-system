...
try {
  ...
  throw new Error('pdf-parse package is required for CV extraction');
}
    ...
    try {
      if (mimetype.includes('pdf')) {
        ...
        try {
          ...
          // Fallback to a simpler pdf-parse approach
          try {
            const pdfParse = require('pdf-parse');
            
            return new Promise((resolve, reject) => {
              pdfParse(buffer).then((data: any) => {
                this.logger.log(`Successfully extracted ${data.text?.length || 0} characters from PDF using pdf-parse`);
                resolve(data.text || '');
              }).catch((error: any) => {
        ...
        this.logger.log(`Successfully extracted ${result.value?.length || 0} characters from Word document`);
        return result.value;
      } else {
