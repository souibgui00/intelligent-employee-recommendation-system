...
export class SkillsService {
  ...
  async findOne(id: string): Promise<Skill | null> {
    ...
    return this.skillModel
      .findOne({ name: { $regex: new RegExp(`^${id}$`, 'i') } })
      .exec();
  }
  ...
  async getGlobalSkillsDashboard(): Promise<any> {
    ...
    const skillDocMap = new Map(
      skillDocs.map((doc: any) => [doc._id.toString(), doc]),
    );
    ...
    const topSkills = skillIds
      .map((skillId) => {
        ...
        return {
          ...
          type: doc.type,
          averageScore:
            agg.employeeCount > 0 ? agg.totalScore / agg.employeeCount : 0,
    ...
    for (const item of topSkills) {
      ...
      categoryDistributionMap.set(
        category,
        (categoryDistributionMap.get(category) || 0) + 1,
      );
    }
    ...
    const averageOrganizationScore =
