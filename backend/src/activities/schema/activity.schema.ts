...
export class Activity extends Document {
  ...
  @Prop({
    required: true,
    enum: ['training', 'workshop', 'mentoring', 'webinar'],
  })
  type!: string;
  ...
  @Prop({
    default: 'development',
    enum: ['development', 'performance', 'balanced'],
  })
  intent!: string;
...
ActivitySchema.post('findOneAndDelete', async function (doc: any) {
  if (doc) {
    try {
      await this.model.db
        .model('Participation')
        .deleteMany({ activityId: doc._id });
      await this.model.db
        .model('Assignment')
        .deleteMany({ activityId: doc._id });
      console.log(
        `[Activity Cleanup] Removed participations and assignments for deleted activity: ${doc._id}`,
      );
    } catch (error) {
      console.error(
        '[Activity Cleanup] Failed to run cascading deletes:',
        error,
      );
    }
  }
