...
UserSchema.post('findOneAndDelete', async function (doc: any) {
  if (doc) {
    try {
      ...
      await db
        .model('Assignment')
        .deleteMany({ $or: [{ userId: doc._id }, { assignedBy: doc._id }] });
      await db
        .model('Evaluation')
        .deleteMany({ $or: [{ employeeId: doc._id }, { managerId: doc._id }] });
      await db.model('Notification').deleteMany({ recipientId: doc._id });
      console.log(
        `[User Cleanup] Removed related records for deleted user: ${doc._id}`,
      );
    } catch (error) {
