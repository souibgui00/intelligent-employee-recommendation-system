...
export class UsersService {
  ...
  async create(data: any) {
    ...
    // Sync Department.manager_id so the department page shows the correct manager
    await this.syncDepartmentManager(
      savedUser._id.toString(),
      (userObj as any).role,
      (userObj as any).department_id,
    );
  ...
  /**
   * Keep Department.manager_id in sync with the user's role & department.
   *
   * Rules:
   * - If the user IS a MANAGER and has a department_id → set that department's manager_id to this user.
   * - Otherwise (role changed away from MANAGER, or no department) → clear manager_id on any
   *   department that still points to this user.
   */
  private async syncDepartmentManager(
    userId: string,
    role: string | undefined,
    departmentId: string | Types.ObjectId | null | undefined,
  ): Promise<void> {
    try {
      const DeptModel = this.userModel.db.model('Department');
      const isManager = (role || '').toUpperCase() === Role.MANAGER;

      if (isManager && departmentId) {
        // Set this user as the manager of their department
        await DeptModel.findByIdAndUpdate(departmentId, {
          manager_id: new Types.ObjectId(userId),
        });
        // Guard: also clear themselves from any OTHER department (shouldn't happen,
        // but keeps data clean if someone manually edited the DB)
        await DeptModel.updateMany(
          { _id: { $ne: departmentId }, manager_id: new Types.ObjectId(userId) },
          { $set: { manager_id: null } },
        );
      } else {
        // Not a manager (anymore) → remove this user from whichever department still lists them
        await DeptModel.updateMany(
          { manager_id: new Types.ObjectId(userId) },
          { $set: { manager_id: null } },
        );
      ...
      console.error('[UsersService] syncDepartmentManager failed:', err.message);
    }
  }
  ...
  async update(id: string, data: UpdateUserDto) {
    ...
    // Fetch the current role before the update (role is stripped from updateData above)
    const currentUser = await this.userModel.findById(id).select('role department_id').lean();
    const effectiveRole = (currentUser as any)?.role;
    ...
    // Sync Department.manager_id after a profile update
    const newDeptId = updateData.department_id ?? (currentUser as any)?.department_id;
    await this.syncDepartmentManager(id, effectiveRole, newDeptId);
  ...
  async updateRole(id: string, role: Role) {
    ...
    // Sync Department.manager_id when a user's role is explicitly changed
    const rawDeptId = (user as any).department_id?._id ?? (user as any).department_id;
    await this.syncDepartmentManager(id, role, rawDeptId);
