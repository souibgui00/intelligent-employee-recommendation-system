...
export class AssignmentsController {
  ...
  async forwardToManager(
    ...
    return this.assignmentsService.forwardToManager(
      forwardDto,
      req.user.userId,
    );
  }
  ...
  async forwardToDepartmentManager(
    ...
    if (
      !Array.isArray(forwardDto?.candidateIds) ||
      forwardDto.candidateIds.length === 0
    ) {
    ...
    return this.assignmentsService.forwardToDepartmentManagers(
      forwardDto,
      req.user.userId,
    );
  }
  ...
  async getAssignments(
    ...
    if (role === 'admin' || role === 'hr') {
      ...
      const created = await this.assignmentsService.findByAssigner(
        req.user.userId,
        { page, limit, select },
      );
      const recommended =
        await this.assignmentsService.findRecommendationsForManager(
          req.user.userId,
          { page, limit, select },
        );
  ...
  @Roles(Role.EMPLOYEE)
  @Post(':id/accept')
  async employeeAccept(@Req() req: any, @Param('id') id: string) {
