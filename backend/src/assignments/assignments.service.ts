...
export class AssignmentsService {
  ...
  async forwardToManager(
    ...
    const { candidateIds, activityId, managerId, aiScore, skillGaps, reason } =
      forwardDto;
    ...
    if (activity.workflowStatus === 'rejected') {
      throw new BadRequestException(
        'Cannot recommend candidates for rejected activity',
      );
    }
    ...
    this.logger.debug(
      `[ForwardToManager] Manager target managerId=${managerId} role=${manager?.role}`,
    );
    if (manager.role !== Role.MANAGER) {
      this.logger.warn(
        `[ForwardToManager] Invalid manager role role=${manager.role} managerId=${managerId}`,
      );
    ...
    const candidateObjectIds = uniqueCandidateIds.map(
    ...
    const existingCandidateIdsSet = new Set(
    ...
    if (candidateCount > 0) {
      const createdCandidateIds = createdAssignments.map((a) =>
        a.userId.toString(),
      );
  ...
  async forwardToDepartmentManagers(
    forwardDto: ForwardToDepartmentManagerDto,
    requesterId: string,
  ): Promise<any> {
  ...
  private async processCandidateToManagerMapping(
    ...
    for (const candidateId of uniqueCandidateIds) {
      ...
      this.logger.log(
        `[ForwardToDepartmentManagers] Linked "${candidate.name}" → Manager "${manager.name}" (${managerId})`,
      );
  ...
  private async createAssignmentsAndNotify(
    ...
    const candidateObjectIds = [...candidateToManager.keys()].map(
      (id) => new Types.ObjectId(id),
    );
    ...
    const existingPairSet = new Set(
      existingAssignments.map(
        (a) => `${a.userId.toString()}::${a.managerId?.toString?.() || ''}`,
      ),
    );
    ...
    for (const [candidateId, managerId] of candidateToManager.entries()) {
      ...
      if (existingPairSet.has(pairKey)) {
        const existing = existingAssignments.find(
          (a) =>
            `${a.userId.toString()}::${a.managerId?.toString?.() || ''}` ===
            pairKey,
        );
        skippedBreakdown.assignmentAlreadyExists += 1;
        skippedCandidates.push({
          candidateId,
          reason: `Recommendation already exists (Status: ${existing?.status || 'Active'})`,
        });
        this.logger.debug(
  ...
  async findByRecipient(
    userId: string,
    options?: { page?: number; limit?: number; select?: string[] },
  ): Promise<Assignment[]> {
    const { page = 1, limit = 20, select } = options || {};
    let query = this.assignmentModel.find({
      userId: new Types.ObjectId(userId),
  ...
  async findRecommendationsForManager(
    managerId: string,
    options?: { page?: number; limit?: number; select?: string[] },
  ): Promise<Assignment[]> {
    const { page = 1, limit = 20, select } = options || {};
  ...
  async updateStatus(
    ...
    const assignmentManagerId = String(
      (assignment.managerId as any)?._id || assignment.managerId || '',
    );
    ...
    const candidateId = String(
      (assignment.userId as any)?._id || assignment.userId || '',
    );
    const activityTitle = (assignment.activityId as any)?.title || 'Activity';
    const activityId = String(
      (assignment.activityId as any)?._id || assignment.activityId || '',
    );
    ...
    try {
      ...
      this.logger.error(
        `[UpdateStatus] Failed to notify candidate assignmentId=${id}: ${err?.message || err}`,
      );
    }
  ...
  async employeeAccept(id: string, employeeId: string): Promise<Assignment> {
    ...
    if (assignment.userId.toString() !== employeeId) {
      throw new ForbiddenException(
        'You can only accept assignments meant for you',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `[EmployeeAccept] Participation created for user=${employeeId} activity=${activityIdString}`,
      );
    } catch (err: any) {
      ...
      if (
        ...
        err?.message?.includes('Cannot re-enroll')
      ) {
        this.logger.log(
          `[EmployeeAccept] Participation already exists (or forbidden) for user=${employeeId}.`,
        );
      } else {
        this.logger.error(
          `[EmployeeAccept] Failed to create participation: ${err.message}`,
        );
    ...
    if (assignment.managerId) {
      try {
        const activityTitle =
          (assignment.activityId as any)?.title || 'the activity';
        ...
        this.logger.error(
          `[EmployeeAccept] Failed to notify manager: ${err.message}`,
        );
      }
    }
  ...
  async employeeReject(
    ...
    if (assignment.userId.toString() !== employeeId) {
      throw new ForbiddenException(
        'You can only decline assignments meant for you',
      );
    }
    ...
    if (assignment.managerId) {
      try {
        ...
        this.logger.error(
          `[EmployeeReject] Failed to notify manager about decline: ${err.message}`,
        );
      }
    }
