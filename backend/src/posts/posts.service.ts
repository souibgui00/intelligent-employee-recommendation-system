...
export class PostsService {
    ...
    private extractTags(content: string): string[] {
        ...
        return [...new Set(matches.map((tag) => tag.slice(1).toLowerCase()))].slice(0, 15);
    }
    ...
    private analyzeSentimentFallback(content: string): { label: 'positive' | 'negative' | 'neutral'; score: number; confidence?: number; source?: string } {
        const text = content.toLowerCase();
        const positiveWords = [
            'good', 'great', 'excellent', 'awesome', 'love', 'nice', 'perfect', 'happy', 'amazing',
            'bien', 'super', 'excellent', 'bravo', 'merci', 'parfait', 'genial', 'heureux', 'top',
        ];
        const negativeWords = [
        ...
        if (score > 0.2) return { label: 'positive', score, confidence: Math.min(1, Math.abs(score)), source: 'fallback' };
        if (score < -0.2) return { label: 'negative', score, confidence: Math.min(1, Math.abs(score)), source: 'fallback' };
        return { label: 'neutral', score, confidence: Math.min(1, Math.abs(score)), source: 'fallback' };
    }

    private async analyzeSentiment(content: string): Promise<{ label: 'positive' | 'negative' | 'neutral'; score: number; confidence?: number; source?: string }> {
        const nlpServiceUrl = process.env.NLP_SERVICE_URL || 'http://localhost:8000';

        try {
            const response = await firstValueFrom(
                this.httpService.post<{ label?: string; score?: number; confidence?: number }>(`${nlpServiceUrl}/analyze-sentiment`, {
                    text: content,
                }),
            );
            ...
            const confidence = typeof response?.data?.confidence === 'number' ? response.data.confidence : undefined;

            if (label === 'positive' || label === 'negative' || label === 'neutral') {
                return { label, score, confidence, source: 'nlp-service' };
            }
        } catch (error: any) {
            console.warn('[PostsService] NLP sentiment unavailable, falling back to local analyzer:', error?.message || error);
        }
    ...
    private getAdvancedSort(sort?: 'latest' | 'top' | 'discussed'): Record<string, 1 | -1> {
        if (sort === 'top') {
            return { isPinned: -1, likesCount: -1, commentsCount: -1, createdAt: -1 } as const;
        }

        if (sort === 'discussed') {
            return { isPinned: -1, commentsCount: -1, likesCount: -1, createdAt: -1 } as const;
        }
    ...
    async sharePost(postId: string, userData: { userId: string; authorName: string; authorAvatar?: string; content?: string }): Promise<Post> {
        ...
        const sharedPost = new this.postModel({
            ...
            tags: [...new Set([...this.extractTags(finalContent), ...(sourcePost.tags || [])])].slice(0, 15),
            isShare: true,
    ...
    async getUserPublications(userId: string): Promise<Post[]> {
        return this.postModel
            .find({ authorId: this.toObjectId(userId, 'userId'), isShare: { $ne: true } })
    ...
    async getUserProfileFeed(userId: string) {
        ...
        const [publications, shares, likedReceivedAgg, commentsReceivedAgg] = await Promise.all([
    ...
    async update(id: string, userId: string, role: string | undefined, updatePostDto: UpdatePostDto): Promise<Post> {
        const post = await this.findOne(id);
        ...
        const updated = await this.postModel.findByIdAndUpdate(id, payload, { new: true }).exec();
        if (!updated) {
            throw new NotFoundException(`Post with ID ${id} not found`);
        }
