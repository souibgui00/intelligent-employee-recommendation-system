...
export class FaceRecognitionService {
    ...
    async getFaceProfile(email: string) {
        const user = await this.userModel.findOne({ email }).select('name email facePicture isFaceIdEnabled avatar');
        if (!user) {
    ...
    async disableFaceId(userId: string) {
        return this.userModel.findByIdAndUpdate(userId, { isFaceIdEnabled: false }, { new: true }).select('-password');
    }
}
