...
export function EmployeeProfile({ employee: initialEmployee, onClose }) {
  ...
  const employee = useMemo(() => {
    ...
    // Always prefer the fully-fetched API version (lightweight === false).
    // The store entry uses lightweight=true and has unpopulated skillId strings,
    // so if we fall back to it the skill buckets resolve to null and show nothing.
    if (initialEmployee.lightweight === false) {
    ...
    // Use whichever has more skills
    return initialSkills >= storeSkills ? initialEmployee : storeEmployee
