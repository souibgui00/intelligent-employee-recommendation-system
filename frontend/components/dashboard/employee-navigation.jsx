            ...
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-10">
                    <Link to="/employee" className="flex items-center gap-3 group">
                        <div className="w-10 h-10 bg-[#F28C1B] rounded-xl flex items-center justify-center shadow-lg shadow-[#F28C1B]/20 group-hover:rotate-6 transition-transform">
                        ...
                        <div className="hidden md:flex flex-col">
                            ...
                            <span className={cn(
                                ...
                                showSolidNav ? "text-[#F28C1B]/80" : "text-white/70"
                            )}>
                            ...
                            return (
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        isActive
                                            ? "text-[#F28C1B] bg-[#F28C1B]/5"
                                            : showSolidNav ? "text-slate-600 hover:text-[#1E5FA8] hover:bg-slate-50" : "text-white/80 hover:text-white"
                                    )}
                                >
                                    {item.name}
                                    {isActive && (
                                        <div className="absolute bottom-0 left-4 right-4 h-1 bg-[#F28C1B] rounded-full" />
                                    )}
                                </Link>
                            )
                        })}
                        ...
                        <Search className={cn(
                         ...
                         <Input
                            ...
                            className={cn(
                                ...
                                showSolidNav
                                    ? "bg-slate-100 focus:bg-white focus:ring-4 focus:ring-[#1E5FA8]/10 w-80 shadow-inner text-slate-900 placeholder:text-slate-400"
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            ...
                            >
                                ...
                                {unreadCount > 0 && (
                                    <span className="absolute top-2.5 right-2.5 w-4 h-4 rounded-full bg-[#1E5FA8] text-[10px] font-black text-white flex items-center justify-center border-2 border-white animate-pulse shadow-lg shadow-blue-500/20">
                                        {unreadCount}
                                    </span>
                                )}
                            </Button>
                        ...
                        <DropdownMenuContent align="end" className="w-80 p-0 border-none rounded-2xl shadow-2xl overflow-hidden mt-2">
                            <div className="bg-[#2C2C2C] px-5 py-4 flex items-center justify-between">
                                <span className="text-[10px] font-black text-[#F28C1B] tracking-widest">Notifications / alerts</span>
                                {unreadCount > 0 && (
                                    <button
                                        onClick={() => markAllNotificationsRead(user.id)}
                                        className="text-[10px] font-bold text-white/40 hover:text-[#F28C1B] transition-colors"
                                    >
                                        Mark all as read
                                    </button>
                                )}
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-2 group outline-none">
                                <Avatar className="h-10 w-10 border-2 border-transparent group-hover:border-[#1E5FA8] transition-all">
                                    <AvatarImage src={user?.avatar} />
                                    <AvatarFallback className="bg-[#1E5FA8] text-white font-bold">{getInitials(user?.name)}</AvatarFallback>
                                </Avatar>
                        ...
                        <DropdownMenuContent align="end" className="w-72 p-0 border-slate-100 rounded-3xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] mt-4 overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                            ...
                            <div className="relative p-6 overflow-hidden">
                                ...
                                <div className="relative flex items-center gap-4 mb-4">
                                    <Avatar className="h-12 w-12 border-2 border-[#F28C1B]/30 ring-4 ring-[#F28C1B]/5 shadow-2xl">
                                        <AvatarImage src={user?.avatar} />
                                        <AvatarFallback className="bg-[#F28C1B] text-white font-black">{getInitials(user?.name)}</AvatarFallback>
                                    </Avatar>
                                    <div className="flex flex-col min-w-0">
                                        <p className="text-white font-black text-sm tracking-tight truncate leading-tight">{user?.name}</p>
                                        <p className="text-[#F28C1B] font-bold text-[10px] tracking-[0.2em] mt-1 shrink-0">{(user?.role || "employee").charAt(0).toUpperCase() + (user?.role || "employee").slice(1).toLowerCase()}</p>
                                ...
                                <div className="relative space-y-2 py-3 px-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10">
                                    ...
                                    <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                                        <div className="h-full w-[84%] bg-gradient-to-r from-[#F28C1B] to-orange-400 rounded-full shadow-[0_0_12px_rgba(242,140,27,0.4)]" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-3 bg-white">
                                <div className="space-y-1">
                                    <DropdownMenuItem onClick={() => navigate("/employee/profile")} className="flex items-center gap-3 px-4 py-3.5 text-xs font-black text-slate-600 tracking-widest rounded-2xl hover:bg-slate-50 hover:text-[#1E5FA8] transition-all cursor-pointer group outline-none border border-transparent hover:border-[#1E5FA8]/20">
                                        <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center group-hover:bg-blue-50 group-hover:scale-110 transition-all shadow-sm">
                                            <User className="w-4 h-4 text-slate-400 group-hover:text-[#1E5FA8]" />
                                        </div>
                                        My profile
                                    </DropdownMenuItem>

                                    <DropdownMenuItem onClick={() => navigate("/employee/progress")} className="flex items-center gap-3 px-4 py-3.5 text-xs font-black text-slate-600 tracking-widest rounded-2xl hover:bg-slate-50 hover:text-[#1E5FA8] transition-all cursor-pointer group outline-none border border-transparent hover:border-[#1E5FA8]/20">
                                        <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center group-hover:bg-blue-50 group-hover:scale-110 transition-all shadow-sm">
                                            <Sparkles className="w-4 h-4 text-slate-400 group-hover:text-[#1E5FA8]" />
                                        </div>
                                        My achievements
                                    </DropdownMenuItem>
                                </div>
