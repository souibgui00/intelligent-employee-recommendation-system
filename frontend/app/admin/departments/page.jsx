                                ...
                                <div className="lg:col-span-2 space-y-10">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {(() => {
                                                const viewId = String(viewingDept.id || viewingDept._id)
                                                const deptEmps = (employees || []).filter(e => {
                                                    const isSameDept = e.department_id && String(e.department_id) === viewId
                                    ...
                                    <div className="p-8 rounded-[2.5rem] bg-white border-2 border-slate-50 shadow-premium">
                                        ...
                                        <div className="flex items-end gap-6 mb-4">
                                            <p className="text-6xl font-black text-slate-900 tracking-tighter leading-none">
                                                {employees?.filter(e => e.department_id && String(e.department_id) === String(viewingDept.id || viewingDept._id)).length}
                                            </p>
                ...
                <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
                    {filteredDepts.map((dept) => {
                        ...
                        const deptEmployees = employees?.filter(e =>
                            e.department_id && String(e.department_id) === deptId
                        ) || []

                        const targetManagerId = typeof dept.manager_id === 'string'
                            ? dept.manager_id
                            : dept.manager_id?._id || dept.manager_id?.id
                        const managerObj = employees?.find(e =>
                            targetManagerId &&
                            (String(e.id) === String(targetManagerId) ||
                                String(e._id) === String(targetManagerId))
                        )
