...
import {
  Users, Calendar, TrendingUp, ArrowRight, DatabaseZap, Loader2, Activity, History,
} from "lucide-react"
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        const [stats, departments, activities] = await Promise.all([
          api.get("/api/dashboard/stats"),
          api.get("/api/departments?limit=10"),
          api.get("/api/activities?limit=10"),
        ])
        setDashboardData({ stats, departments, activities })
      } catch (err) {
  ...
  if (!dashboardData) return <PanelLoader />
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
            <div className="bg-white rounded-3xl p-8 shadow-premium border border-slate-50">
              ...
              <Suspense fallback={<PanelLoader />}>
                <DepartmentOverview departments={dashboardData.departments} hideHeader={true} />
              </Suspense>
            </div>
          </div>
          <div className="space-y-10">
            <div className="bg-white rounded-3xl p-8 shadow-premium border border-slate-50">
              ...
              <Suspense fallback={<PanelLoader />}>
                <UpcomingActivities activities={dashboardData.activities} hideHeader={true} />
              </Suspense>
